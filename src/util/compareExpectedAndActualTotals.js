...
export default function compareExpectedAndActualTotals(actualTotal, expectedTotal) {
  ...
  // We use currency exchange and hence are ready to tolerate some error
  const expectedTotalString = accounting.toFixed(expectedTotal, 2);
  const actualTotalString = accounting.toFixed(actualTotal, 2);
