 ...
 */
export default async function assignOrderToFulfillmentManager(context, input) {
  ...
  if(groupExists.name!=="fulfillment manager") throw new ReactionError("invalid-group", "Invalid group is being assigned");
  ...
  const modifier = {
    $set: {
      fulfillmentManager: accountExists._id,
    }
  };
